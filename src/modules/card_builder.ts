...
async function BuildCards(
  ...
  cardData.forEach((entry: any) => {
    ...
    for (let element in entry) {
      let occurrences = card.window.document.querySelectorAll(
        `[varname=${element}]`
      );
      occurrences.forEach((occurence: any) => {
        occurence.innerHTML = entry[element];
      });
    }

    base.window.document.documentElement
      .querySelector("body")
      ?.appendChild(card.window.document.querySelector(".card") as Node);
  });

  fs.mkdirSync("./build/" + name + "/", { recursive: true });
  ...
  if (generateImage) {
    ...
    cards.forEach(async (card: any, i: any) => {
